      ...
      <div className="relative z-20 w-full max-w-[1400px] flex justify-center items-center perspective-2000 p-1 md:p-8 rounded-lg">
        ...
        >
          ...
          <Page isLeft={false} number="01">
            <div className="h-full flex flex-col">
              <div className="mb-12 text-center mt-6">
                <span className="font-sans text-[9px] tracking-[0.2em] font-bold text-[#8c6b4f] uppercase block mb-3">Chapter I</span>
                <h3 className="font-serif text-4xl font-normal text-[#2c241b]">Index</h3>
              </div>

              <div className="space-y-6 px-4 md:px-8 flex-grow">
                {directoryItems.map((category, idx) => (
                  <div 
                    key={idx} 
                    onClick={() => flipToPage(category.targetIndex)}
                    className="group flex items-baseline justify-between cursor-pointer w-full"
                  >
                    <div className="flex items-baseline space-x-4 flex-grow bg-transparent relative z-10 w-full overflow-hidden">
                      <span className="font-serif text-sm md:text-base font-normal text-[#2c241b] group-hover:text-[#8c6b4f] transition-colors whitespace-nowrap bg-[#fdfaf6] pr-3 z-10">
                        {category.title}
                      </span>
                      {/* Dotted Leader */}
                      <span className="absolute inset-x-0 bottom-[5px] border-b border-dotted border-[#2c241b]/20 z-0"></span>
                    </div>
                    <span className="font-serif text-sm md:text-base font-normal text-[#8c6b4f] bg-[#fdfaf6] pl-3 z-10">
                      {category.pageNum}
                    </span>
                  </div>
                ))}
          ...
          <Page isLeft={true} number="02">
            <div className="h-full flex flex-col pt-8 px-2 md:px-6">
              <div className="text-center mb-10">
                <h3 className="font-serif text-3xl md:text-4xl font-normal text-[#2c241b] mb-4">Our Heritage</h3>
                <div className="mx-auto w-8 h-px bg-[#8c6b4f]/40"></div>
              </div>

              <div className="flex-grow">
                <p className="font-serif text-xl italic text-[#5c4a3d] text-center leading-relaxed mb-10 px-4">
                  "Tradition gently folded into every layer."
                </p>
                <div className="font-sans font-light text-xs md:text-sm text-[#4a3e35] text-justify leading-loose">
                  <span className="font-serif text-[3.5rem] float-left mr-3 mt-1 text-[#2c241b] leading-[0.7]">P</span>
              ...
              <div className="border-t border-[#2c241b]/10 pt-6 mt-8">
                <p className="font-serif text-center text-[10px] uppercase tracking-widest text-[#8c6b4f]">Established MMXIV</p>
              </div>
            </div>
          </Page>

          {/* Page 3 (Right): SPREAD 0 RIGHT: Cinnamon Rolls Set */}
          <Page isLeft={false} number="03">
            <div className="h-full flex flex-col pt-8">
              <div className="text-center mb-8">
                <h3 className="font-serif text-3xl md:text-4xl font-normal text-[#2c241b]">Signature Sets</h3>
                <p className="font-serif italic text-[#8c6b4f] text-sm mt-3">Curated for sharing</p>
              </div>

              <div className="space-y-6 flex-grow px-2 md:px-4">
                {[
                  ...
                  { title: '4 Swirl of Premium', desc: 'Premium assortment of Lotus Biscoff and rich hazelnut.', price: '34.00', image: '/4swirlofpremium.png' },
                  { title: '4 Chocolate Hideout', desc: 'A pure chocolate affair of Nutella and Ovomaltine.', price: '34.00', image: '/4chocolatehideout.png' },
                ].map((set, idx) => (
                  <div key={idx} className="flex items-start gap-4 p-2 rounded-xl border border-transparent hover:border-[#2c241b]/5 transition duration-300">
                    <img src={set.image} alt={set.title} className="w-16 h-16 rounded-full object-cover shadow-[0_4px_10px_rgba(0,0,0,0.08)] bg-white/50 border border-white" />
                    <div className="flex-grow pt-1.5 border-b border-[#2c241b]/10 pb-4">
                      <div className="flex justify-between items-baseline mb-1">
                        <h4 className="text-sm font-serif font-bold text-[#2c241b] tracking-wide">{set.title}</h4>
                        <span className="text-[10px] font-sans tracking-wider text-[#8c6b4f]">RM {set.price}</span>
                      </div>
                      <p className="text-xs font-serif italic text-[#5c4a3d] leading-relaxed">{set.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Page isLeft={true} number="04">
            <div className="h-full flex flex-col pt-8 px-2 md:px-4 overflow-y-auto custom-scrollbar">
              
              {/* Set Rahmah section */}
              <div className="mb-8">
                <div className="text-center mb-6">
                  <h3 className="font-serif text-xl font-bold text-[#2c241b]">Set Rahmah</h3>
                  <div className="mx-auto w-6 h-px bg-[#8c6b4f] mt-2 mb-1"></div>
                  <p className="font-serif italic text-[10px] text-[#8c6b4f]">Accessible Indulgence</p>
                </div>
                
                <div className="space-y-4">
                  {[
                    { title: 'Rahmah Happiness', price: '26.00', image: '/4swirlofhappiness.png', desc: 'Less sweet assorted flavors.' },
                    { title: 'Rahmah Premium', price: '27.00', image: '/4swirlofpremium.png', desc: 'Selected fresh baked assortment.' },
                  ].map((set, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <img src={set.image} alt={set.title} className="w-10 h-10 rounded-full object-cover shadow border border-white" />
                      <div className="flex-grow border-b border-[#2c241b]/5 pb-2">
                        <div className="flex justify-between items-center">
                          <span className="font-serif text-sm text-[#2c241b]">{set.title}</span>
                          <span className="text-[10px] font-sans tracking-wider text-[#8c6b4f]">RM {set.price}</span>
                        </div>
                        <p className="text-[10px] font-serif italic text-[#8c6b4f] leading-none mt-1">{set.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="flex-grow pt-4">
                <h4 className="font-sans text-[10px] tracking-[0.2em] uppercase font-bold text-[#2c241b] text-center mb-5">Single Delicacies</h4>
                <div className="grid grid-cols-1 gap-y-3 gap-x-6 text-[11px] font-serif">
                  {[
          ...
          <Page isLeft={false} number="05">
            <div className="h-full flex flex-col pt-8 px-2 md:px-6 relative overflow-hidden">
              ...
              <div className="relative z-10 text-center mb-8">
                <h3 className="font-serif text-3xl font-normal text-[#2c241b] tracking-wide">Gourmet Bagels</h3>
                <div className="mx-auto w-6 h-[2px] bg-[#8c6b4f] mt-3"></div>
              </div>

              <p className="relative z-10 text-center font-sans tracking-[0.1em] text-[8px] uppercase text-[#8c6b4f] font-bold mb-6">Toasted & Ready to Eat</p>

              <div className="relative z-10 space-y-4 px-2 overflow-y-auto custom-scrollbar flex-grow pb-8">
                {[
                  ...
                  <div key={idx} className="flex justify-between items-end border-b border-[#2c241b]/10 pb-2">
                    <div>
                      ...
                      <span className="font-serif italic text-[10px] text-[#8c6b4f]">{item.detail}</span>
                    </div>
                    <span className="font-sans text-[10px] font-semibold text-[#8c6b4f] tracking-wider mb-0.5">{item.price}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Page isLeft={true} number="06">
            <div className="h-full flex flex-col pt-8 px-4 md:px-8">
              <div className="text-center mb-10">
                <span className="font-sans text-[9px] font-bold tracking-[0.2em] text-[#8c6b4f] uppercase mb-2 block">Our Process</span>
                <h3 className="font-serif text-3xl text-[#2c241b]">The Baker's Craft</h3>
              </div>

              <div className="flex-grow space-y-10">
                <div className="border border-[#2c241b]/10 p-6 relative">
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#fdfaf6] px-4 font-serif italic text-lg text-[#2c241b]">Sourdough</div>
                  <p className="font-sans font-light text-xs text-center text-[#4a3e35] leading-relaxed">
                ...
                <div className="grid grid-cols-2 gap-x-8 gap-y-8 items-start pt-4">
                  <div className="text-center">
                    <span className="text-2xl font-serif text-[#2c241b] block mb-2">24h</span>
                    <div className="w-4 h-[1px] bg-[#8c6b4f] mx-auto mb-2" />
                    <span className="text-[9px] font-sans font-bold uppercase tracking-widest text-[#8c6b4f]">Cold Proof</span>
                  </div>
                  <div className="text-center">
                    <span className="text-2xl font-serif text-[#2c241b] block mb-2">100%</span>
                    <div className="w-4 h-[1px] bg-[#8c6b4f] mx-auto mb-2" />
                    <span className="text-[9px] font-sans font-bold uppercase tracking-widest text-[#8c6b4f]">Pure Butter</span>
                  </div>
                  <div className="text-center">
                    <span className="text-2xl font-serif text-[#2c241b] block mb-2">Ceylon</span>
                    <div className="w-4 h-[1px] bg-[#8c6b4f] mx-auto mb-2" />
                    <span className="text-[9px] font-sans font-bold uppercase tracking-widest text-[#8c6b4f]">Cinnamon</span>
                  </div>
                  <div className="text-center">
                    <span className="text-2xl font-serif text-[#2c241b] block mb-2">Hand</span>
                    <div className="w-4 h-[1px] bg-[#8c6b4f] mx-auto mb-2" />
          ...
          <Page isLeft={false} number="07">
            <div className="h-full flex flex-col pt-8 px-2 md:px-6">
              <div className="text-center mb-6">
                <span className="font-sans text-[9px] tracking-[0.2em] font-bold text-[#8c6b4f] uppercase block mb-2">Experience</span>
                <h3 className="font-serif text-3xl font-normal text-[#2c241b]">Bespoke Box</h3>
                <p className="font-serif italic text-xs text-[#5c4a3d] mt-2 border-b border-[#2c241b]/10 pb-4 inline-block">
                  Curate 6 pieces. Enjoy a 10% privilege.
                </p>
              </div>

              <div className="space-y-4 flex-grow overflow-y-auto custom-scrollbar px-2 pb-4">
                  ...
                  return (
                    <div key={roll.id} className="flex justify-between items-center group">
                      <div className="flex items-center gap-3">
                         <div className="w-10 h-10 rounded-full border border-[#2c241b]/10 p-0.5 overflow-hidden">
                           <img src={roll.image} alt={roll.title} className="w-full h-full object-cover rounded-full mix-blend-multiply" />
                         </div>
                         <div>
                           <span className="font-serif text-sm text-[#2c241b] block">{roll.title.replace(' Swirl', '')}</span>
                           <span className="font-sans text-[9px] font-semibold tracking-wider text-[#8c6b4f]">{roll.price}</span>
                         </div>
                      </div>

                      <div className="flex items-center space-x-3 bg-white/50 px-2 py-1 border border-[#2c241b]/5 rounded-sm">
                        <button onClick={() => updateBoxCount(roll.id, -1)} className="font-serif text-[#8c6b4f] hover:text-[#2c241b] transition px-1">-</button>
                        <span className="font-sans text-[10px] w-3 text-center">{quantity}</span>
          ...
          <Page isLeft={true} number="08">
            <div className="h-full flex flex-col pt-8 px-4 md:px-8">
              <div className="text-center mb-6">
                <h3 className="font-serif text-3xl text-[#2c241b]">Gourmet Cookies</h3>
                <div className="mx-auto w-8 h-px bg-[#8c6b4f]/40 mt-4 mb-2"></div>
                <p className="font-serif italic text-sm text-[#8c6b4f]">Warm, gooey elegance.</p>
              </div>

              {/* Video container */}
              <div className="w-full h-48 md:h-[220px] rounded-sm overflow-hidden mb-8 border border-[#2c241b]/10 bg-black/5">
              ...
              <div className="space-y-5 flex-grow">
                {[
                  ...
                  { name: 'Matcha Blossom', price: '7.50', desc: 'Premium matcha green tea baked soft' },
                  { name: 'Pistachio Kunafeh', price: '9.00', desc: 'Middle-eastern pistachio crunch' },
                ].map((item, idx) => (
                  <div key={idx} className="flex justify-between items-start border-b border-[#2c241b]/10 pb-3">
                    <div>
                      <span className="font-serif text-sm font-bold text-[#2c241b] block">{item.name}</span>
                      <span className="font-serif text-xs italic text-[#5c4a3d] mt-1 block">{item.desc}</span>
                    </div>
                    <span className="font-sans text-[10px] font-semibold text-[#8c6b4f] tracking-widest pt-1">{item.price}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Page isLeft={false} number="09">
            <div className="h-full flex flex-col pt-8 px-4 md:px-8">
              <div className="text-center mb-10">
                <h3 className="font-serif text-3xl text-[#2c241b]">Companions</h3>
                <div className="mx-auto w-8 h-px bg-[#8c6b4f]/40 mt-4 mb-2"></div>
              </div>

              <div className="flex-grow space-y-12">
                <div>
                  <h4 className="font-sans text-[9px] font-bold tracking-[0.2em] uppercase text-[#8c6b4f] mb-6 border-b border-[#2c241b]/10 pb-2">Sliced Cakes</h4>
                  <div className="space-y-4">
                    <div className="flex justify-between items-baseline border-b border-dotted border-[#2c241b]/20 pb-2">
                        <span className="font-serif text-sm text-[#2c241b]">Burnt Cheesecake</span>
                        <span className="font-sans text-[9px] font-semibold text-[#8c6b4f] pl-4">15.00</span>
                    </div>
                    <div className="flex justify-between items-baseline border-b border-dotted border-[#2c241b]/20 pb-2">
                        <span className="font-serif text-sm text-[#2c241b]">Premium Spiced Carrot Cake</span>
                        <span className="font-sans text-[9px] font-semibold text-[#8c6b4f] pl-4">14.00</span>
                    </div>
                  </div>
                </div>

                <div>
                  <h4 className="font-sans text-[9px] font-bold tracking-[0.2em] uppercase text-[#8c6b4f] mb-6 border-b border-[#2c241b]/10 pb-2">Beverages</h4>
                  <div className="space-y-4">
                    <div className="flex justify-between items-baseline border-b border-[#2c241b]/5 pb-2">
                        <span className="font-serif text-sm text-[#2c241b]">Americano <span className="text-[9px] font-serif italic text-gray-500">(Hot/Iced)</span></span>
                        <span className="font-sans text-[9px] font-semibold text-[#8c6b4f] pl-4">8.00</span>
                    </div>
                    <div className="flex justify-between items-baseline border-b border-[#2c241b]/5 pb-2">
                        <span className="font-serif text-sm text-[#2c241b]">Latte <span className="text-[9px] font-serif italic text-gray-500">(Hot/Iced)</span></span>
                        <span className="font-sans text-[9px] font-semibold text-[#8c6b4f] pl-4">10.00</span>
                    </div>
                    <div className="flex justify-between items-baseline border-b border-[#2c241b]/5 pb-2">
                        <span className="font-serif text-sm text-[#2c241b]">Matcha Latte <span className="text-[9px] font-serif italic text-gray-500">(Hot/Iced)</span></span>
                        <span className="font-sans text-[9px] font-semibold text-[#8c6b4f] pl-4">12.00</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <Page isLeft={true} number="10">
              <div className="h-full flex flex-col pt-8 px-4 md:px-8">
                 <div className="my-auto flex flex-col items-center text-center">
                   <div className="mb-8 p-1 border border-[#2c241b]/10 rounded-full">
                     <div className="w-16 h-16 rounded-full border border-[#2c241b] flex items-center justify-center">
                       <Sparkles className="w-6 h-6 text-[#8c6b4f]" />
                     </div>
                   </div>
                   
                   <h2 className="font-serif text-3xl text-[#2c241b] mb-6">Gratitude</h2>
                   <p className="font-serif text-sm italic text-[#5c4a3d] leading-loose max-w-[240px] mb-10">
                   ...
                   <button 
                     ...
                     className="border border-[#2c241b] text-[#2c241b] px-8 py-3 font-sans text-[9px] tracking-[0.2em] uppercase transition-colors hover:bg-[#2c241b] hover:text-white"
                   >
                     Initialize Order
                   </button>
                   
                   <p className="text-[8px] mt-16 font-sans text-[#2c241b]/40 tracking-widest uppercase">Lily Cups Bakery &copy; MMXIV</p>
                 </div>
              </div>
          </Page>
